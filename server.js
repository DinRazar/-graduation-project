            ...
            children: selectedTerms.flatMap(term => {
                ...
                return [
                    ...
                    new Paragraph({
                        ...
                        children: [
                            new TextRun({
                                text: `${term.ГОСТ || 'ГОСТ не указан'}. ${term.Наименование 
                                || 'Нет наименования'} Дата введения ${formattedDate}. – М.: Стандартинформ. – ${term.Страниц || 'X'} с.`,
                                font: "Times New Roman",
                                size: 28
                            })
                        ]
...
// app.get('/terms', async (req, res) => {
//     try {
//         const [rows] = await db.query("SELECT * FROM terms ORDER BY term ASC");
//         res.json(rows);
//     } catch (err) {
//         console.error(err);
//         res.status(500).json({ error: 'Ошибка при запросе к БД' });
...
//     try {
//         const query = req.query.q.toLowerCase();
//         const [rows] = await db.query("SELECT * FROM terms WHERE LOWER(term) LIKE ?", [`%${query}%`]);
//         res.json(rows);
//     } catch (err) {
//         console.error(err);
//         res.status(500).json({ error: 'Ошибка при запросе к БД' });
...
//     try {
//         const id = req.params.id;
//         const [rows] = await db.query("SELECT * FROM terms WHERE termID = ?", [id]);
//         if (rows.length === 0) return res.status(404).json({ error: 'Термин не найден' });
//         res.json(rows[0]);
//     } catch (err) {
//         console.error(err);
...
//         sections: [{
//             children: selectedTerms.flatMap(term => {
//                 const formattedDate = term.introduction_date
//                     ? new Date(term.introduction_date).toISOString().split('T')[0]
//                     : 'не указана';
...
//                         children: [
//                             new TextRun({
//                                 text: `${term.term} - ${term.definition || 'Нет данных'}`,
//                                 font: "Times New Roman",
//                                 size: 28
//                             })
//                         ]
...
//                         children: [
//                             new TextRun({
//                                 text: `${term.gost || 'ГОСТ не указан'}. ${term.name || 'Нет наименования'} Дата введения ${formattedDate}. – М.: Стандартинформ. – ${term.page_count || 'X'} с.`,
//                                 font: "Times New Roman",
//                                 size: 28
//                             })
//                         ]
...
//     try {
//         const [rows] = await db.query('SELECT * FROM admin WHERE login = ?', [username]);

//         if (rows.length === 0) return res.status(401).json({ error: 'Неверные данные' });
...
//     try {
//         const [rows] = await db.query('SELECT * FROM feedback ORDER BY feedbackID DESC');
//         res.json(rows);
//     } catch (err) {
//         console.error(err);
//         res.status(500).json({ error: 'Ошибка при запросе сообщений' });
...
//         await db.query('INSERT INTO terms (term, definition, gost, name, introduction_date, page_count) VALUES (?, ?, ?, ?, ?, ?)', [term, definition, gost, name, introduction_date, page_count]);
//         res.json({ success: true });
//     } catch (err) {
//         console.error(err);
//         res.status(500).json({ error: 'Ошибка при добавлении термина' });
...
//         await db.query('UPDATE terms SET term = ?, definition = ?, gost = ? WHERE termID = ?', [term, definition, gost, id]);
//         res.json({ success: true });
//     } catch (err) {
//         console.error(err);
//         res.status(500).json({ error: 'Ошибка при обновлении термина' });
...
//         const [result] = await db.query('DELETE FROM terms WHERE termID = ?', [id]);
//         if (result.affectedRows > 0) {
//             res.json({ success: true });
//         } else {
//             res.status(404).json({ error: 'Термин не найден' });
